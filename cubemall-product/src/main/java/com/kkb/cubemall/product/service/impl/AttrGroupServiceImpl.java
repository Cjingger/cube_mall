...
public class AttrGroupServiceImpl extends ServiceImpl<AttrGroupDao, AttrGroupEntity> implements AttrGroupService {
    ...
    public List<AttrGroupWithAttrsVo> getAttrGroupWithAttrsByCategoryId(Long categoryId) {
        ...
        List<AttrGroupWithAttrsVo> collect = attrGroupEntities.stream().map(
                attrGroupEntity -> {
                    ...
                    //设置关联的属性
                    List<AttrEntity> relationAttrs = attrService.getRelationAttr(attrsVo.getId());
                    attrsVo.setAttrs(relationAttrs);
                    return attrsVo;
                }
    ...
    public List<SpuAttrGroupVo> getGroupAttr(Long spuId, Long categoryId) {
        GroupAttrParamVo groupAttrParamVo = new GroupAttrParamVo();
        groupAttrParamVo.setSpuId(spuId);
        groupAttrParamVo.setCategoryId(categoryId);
        List<SpuAttrGroupVo> groupAttrVos = this.baseMapper.getGroupAttr(groupAttrParamVo);
        return groupAttrVos;
    }
